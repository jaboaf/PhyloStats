...
# 1) Let \Theta = \mathbb{N}\^4 be the parameter space. Each \theta \in \Theta determines an equivalence class of B^\star where there are \theta^1=\theta_A 'A's, \theta^2=\theta_C 'C's,\theta^3=\theta_G 'G's, and \theta^4=\theta_T 'T's. The equivalence class is the set of all rearrangements of (A)^{\otimes\theta_A}\otimes(C)^{\otimes\theta_C}\otimes(G)^{\otimes\theta_G}\otimes(T)^{\otimes\theta_T} = \bar{B}^{\theta_{\bar{B}}} is \mathfrak{S}^{|\theta|} \bar{B}^{\theta_{\bar{B}}}. And \Theta partitions B^\star because for any w \in B^\star, N_{\bar{B}}w \in \mathbb{N}^4 means that N_{\bar{B}}w = \vartheta for some \vartheta\in\Theta.
...
# Why don't we just define a model using \mathscr{M}:\Omega\times\Theta\to\mathbb{R}_+? I'm not exactly sure.
